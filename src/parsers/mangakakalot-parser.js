...
class MangakakalotParser extends BaseParser {
    constructor() {
    ...
    async search(query) {
        ...
        try {
            ...
            const items = $('.story_item, .search-story-item, .list-story-item');

            items.each((i, element) => {
                const $el = $(element);
                const $link = $el.find('.story_name a, h3 a, .item-title a, a').first();
    ...
    async getChapters(mangaUrl) {
        try {
            ...
            const chapterElements = $('.chapter-list .row, .row-content-chapter li, ul.row-content-chapter li');
...
module.exports = MangakakalotParser;